...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall: #check if walls aren't hit
                if nextCoord in self.corners:
                    if nextCoord not in corners:
                        corners = list(corners)
                        corners.append(nextCoord)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in corners:
        if corner not in state[1]:
            maxd = max(maxd, mazeDistance(state[0], corner, problem.gamestate))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodGrid.asList():
        maxd = max(maxd, mazeDistance(position, food, problem.startingGameState))
    return maxd
